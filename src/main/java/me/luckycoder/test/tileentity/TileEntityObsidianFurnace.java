package me.luckycoder.test.tileentity;

import blocks.BlockObsidianFurnace;
import init.ModCapabilities;
import me.luckycoder.test.capabilties.Worker;
...
public class TileEntityObsidianFurnace extends TileEntitySidedInventory implements ITickable {
	...
	public TileEntityObsidianFurnace() {
		...
		this.burnWorker = new Worker(0, () -> {
			...
			if (!this.world.isRemote) {
				...
				this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).withProperty(BlockObsidianFurnace.ACTIVATED, false), 3);
			}
		});
		...
		if (!this.world.isRemote) {
			...
			if (canSmelt()) {
				if (this.cookWorker.getMaxWork() == 0 && this.burnWorker.getMaxWork() != 0) {
					this.cookWorker.setMaxCooldown(25);
					this.cookWorker.setReversed(false);
				}
				
				if (TileEntityFurnace.isItemFuel(this.handler.getStackInSlot(2)) && this.burnWorker.getMaxWork() == 0) {
					this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).withProperty(BlockObsidianFurnace.ACTIVATED, true), 3);
